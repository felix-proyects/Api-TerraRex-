...
const pinterestSearchRoute = {
  endpoint: "/api/search/pinterest",
  async run(req, res) {
    const query = (req.query.query || req.body.query || "").trim();
    const type = (req.query.type || req.body.type || "").trim().toLowerCase();
    if (!query) {
      return res.status(400).json({ status: false, creator: "Félix Ofc", error: "Query parameter is required" });
    }
    try {
      ...
      return res.status(500).json({ status: false, creator: "Félix Ofc", error: error.message });
    }
  }
};

module.exports = { pinterestSearchRoute };