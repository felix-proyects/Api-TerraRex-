  ...
  async download({ url: pinUrl }) {
    ...
    try {
      ...
      if (mediaUrls.length === 0) {
        return { status: false, code: 404, result: { message: "No media found for this pin." } };
      }
      return {
        status: true,
        code: 200,
...
const pinterestRoute = {
  endpoint: "/api/download/pinterest",
  async run(req, res) {
    const url = (req.query.url || req.body.url || "").trim();
    if (!url) {
      return res.status(400).json({ status: false, error: "URL parameter is required", code: 400 });
    }
    if (!pinterest.isUrl(url)) {
      return res.status(400).json({ status: false, error: "This is not a valid URL.", code: 400 });
    }
    try {
      const result = await pinterest.download({ url });
      if (!result.status) {
        return res.status(result.code).json({ status: false, error: result.result.message, code: result.code });
      }
      return res.json({ status: true, data: result.result, timestamp: new Date().toISOString() });
    } catch (error) {
...
module.exports = { pinterest, pinterestRoute };