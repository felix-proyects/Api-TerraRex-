...
const securityMiddleware = async (req, res, next) => {
    ...
    if (isApiRoute && !isPublicApi) {
        ...
        try {
            ...
            if (user.requests_today >= user.limit) return res.status(429).json({ status: false, message: "Límite diario alcanzado" });
...
app.listen(PORT, '0.0.0.0', () => console.log(`Api Kazuma corriendo en puerto ${PORT}`));
